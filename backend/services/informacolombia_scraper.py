...
class InformaColombiaScraper:
    ...
    def scrape_company(self, name: str, city: str = "") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            data = {
                ...
                'nit': self._extract_field(profile_soup, r'NIT'),
                'address': self._extract_field(profile_soup, r'Dirección Actual|Dirección|Domicilio'),
                'website': self._extract_website(profile_soup),
                'activity': self._extract_field(profile_soup, r'Actividad|Objeto Social'),
                'employees': self._extract_field(profile_soup, r'Empleados|Rango de Empleados'),
    ...
    def _extract_field(self, soup: BeautifulSoup, pattern: str) -> Optional[str]:
        ...
        label = soup.find(string=re.compile(pattern, re.IGNORECASE))
        if label:
            container = label.find_parent(['div', 'td', 'p', 'li'])
            if container:
